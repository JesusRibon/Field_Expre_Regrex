...
export class FieldRegrexComponent {
  ...
  variable: string = '';
  message: string = '';
  regrex = /^declare\s+([a-zA-Z][\w](?:\s*,\s*[a-zA-Z][\w]{0,14})*)\s+(entero|real|cadena|logico|fecha)\s*;$/
    ...
    if(regrexType.test(getType)){
      ...
      if(typeEntered){
        if(getType === typeEntered[2]){
          ...
          this.message = 'el tipo de dato '+getType+' no fue ingresado por el usuario';
        }
      }
      else if(typeEntered === null){
        this.message = 'Si la estructura ingresada es incorrecta no se mostrara el tipo de dato y variables ingresadas';
      }
    }
    else{
